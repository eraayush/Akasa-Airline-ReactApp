...
function FlightsPage({ bookingDetails, setBookingDetails }) {
  ...
  const chepest = [...flights].sort(
    (a, b) => parseFloat(a.flight.price) - parseFloat(b.flight.price)
  )[0];
  ...
  useEffect(() => {
    if (sortBy === 'price') {
      ...
      flightData.sort((a, b) => {
        const date1 =
          new Date(a.departure.scheduledTime) -
          new Date(a.arrival.scheduledTime);
        const date2 =
          new Date(b.departure.scheduledTime) -
          new Date(b.arrival.scheduledTime);
        return date2 - date1;
      });
  ...
  const sortByList = [
    ...
    ,
    { id: 'depTime', title: 'Departure time' },
  ];
    ...
    <>
      <Box className="w-[80%] m-auto">
        ...
        {flightData.length > 0 ? (
          ...
          <Box className="w-full h-48 flex justify-center items-center border border-[whitesmocke] text-2xl rounded-lg">
            No Flight Found
          </Box>
        )}
      </Box>
